			...
			if(BIT_VALUE(data_bits,ITS_MEAS_FLAG_OVERRIDE_BIT_PLACEMENT)){
					...
					if(data_length > 1){
						...
						if(BIT_VALUE(data_bits,ITS_MEAS_FLAG_SAMP_FREQ_BIT_PLACEMENT)){
								...
								p_iss->p_update_samp_freq(p_iss);
						}
					}					
			}else{
				//set presentation			
 ...
 * @return      Size of encoded data.
 */
static uint8_t iss_measurement_encode(iss_t * p_iss, int32_t * measurement,uint8_t * p_encoded_buffer)
{
    uint8_t flags = 0;
    uint8_t len   = 1;
	
		//Value always present
		len += uint32_encode((((p_iss->IEEE_exponent << 24) & 0xFF000000) | (((*measurement) <<  0) & 0x00FFFFFF)), &p_encoded_buffer[len]);
    ...
    if (p_iss->space_time_present)
    {
        flags |= ITS_MEAS_FLAG_SPACE_TIME_BIT;
				p_encoded_buffer[len++]  = p_iss->coord;
        len   += uint16_encode(++(p_iss->curr_seq_nr), &p_encoded_buffer[len]);
				//Remember, that the above will increament the seq nr so we have a continuous flow
    }
...
static uint32_t add_meas_char(iss_t * p_iss)
{
    ...
    attr_char_value.p_uuid       = &ble_uuid;
    attr_char_value.p_attr_md    = &attr_md;
    attr_char_value.init_len     = iss_measurement_encode(p_iss,&init_value, encoded_measurement);
    attr_char_value.init_offs    = 0;
    attr_char_value.max_len      = MAX_HTM_LEN;
...
uint32_t update_iss_measurement(iss_t * p_iss, int32_t * p_new_meas)
{
    uint32_t err_code = NRF_SUCCESS;
		...
		len = iss_measurement_encode(p_iss,p_new_meas, encoded_measurement);
