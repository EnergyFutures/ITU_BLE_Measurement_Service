...
#define DEVICE_NAME                          "ITUSENSOR2"			                          /**< Name of device. Will be included in the advertising data. */
#define LOCATION_NAME                        "4D21"
#define MANUFACTURER_NAME                    "4D21"						                          /**< Manufacturer. Will be passed to Device Information Service. */
#define MODEL_NUM                            "T1001"     													      /**< Model number. Will be passed to Device Information Service. */
...
itu_sensor_t *sensors[3];
static void registerSensors(void){	
	sensors[0] = getLightSensorTSL2561();
	sensors[1] = getTempSensorTmp36();
	sensors[2] = getTempSensorLmt85();
}
static uint8_t sensors_size = sizeof(sensors) / sizeof(itu_sensor_t *);
...
void ADC_IRQHandler(void)
{
  ...
  NRF_ADC->EVENTS_END = 0;	
	...
	for(uint8_t i = 0; i < sensors_size; i++){
			if(sensors[i]->needs_adc && sensors[i]->adc_done != NULL){
					if(sensors[i]->adc_done(pin)){
						break;
					}
			}
	}	
...
static void advertising_init(void){
    ...
    adv_params.fp          = BLE_GAP_ADV_FP_ANY;
		...
		uint8_t max_sec = sensors_size == 2 ? 4 : 3;
		//we don't want multiple adv before a timeout... hence the 1100
		uint16_t APP_ADV_INTERVAL   = MSEC_TO_UNITS((max_sec/sensors_size) * 1100, UNIT_0_625_MS);           
		uint8_t APP_ADV_TIMEOUT_IN_SECONDS = (max_sec/sensors_size);																     
...
static void its_broadcast_encode_and_set(iss_t * p_iss){
		
		encoded_broadcast_length = 1 + location_name_length;
    if (header.type_present)
    {
				encoded_broadcast[encoded_broadcast_length++] = p_iss->type;
		}
		
    if (header.value_present)
    {
				encoded_broadcast_length += uint32_encode(p_iss->last_measurement, &encoded_broadcast[encoded_broadcast_length]);
    }    
		
    if (header.coor_present)
    {
				encoded_broadcast[encoded_broadcast_length++] = p_iss->coord;
    }
		
		if(storage_struct.current_block > BLOCK_COUNT_X_procent && buffer_full != 1){
			buffer_full = 1;
...
static void advertising_start(void){
	...
	if(current_service_nr == sensors_size ){
			current_service_nr = 0;
	}
	its_broadcast_encode_and_set(sensors[current_service_nr++]->service);
}
...
static void init_ISS_struct (itu_sensor_t * iss_struct, uint32_t * err_code){
		//Initialise ITU Temperature Service
		memset(iss_struct->service, 0, sizeof(*(iss_struct->service)));
		iss_struct->service->IEEE_exponent = ITU_IEEE_FLOAT32_DEFAULT_EXPONENT;
	
		iss_struct->service->curr_seq_nr = 0;
		iss_struct->service->coord = ITS_MEAS_LOCATION_NOT_SET;
		iss_struct->service->space_time_present = false;
	
		//org.bluetooth.unit.thermodynamic_temperature.degree_celsius
		//iss_struct->unit = 0x272F;
		iss_struct->service->unit = 0;
		iss_struct->service->unit_present = false;

		iss_struct->service->type = iss_struct->type;
		iss_struct->service->make = iss_struct->make;
		iss_struct->service->type_make_present = false;
	
		iss_struct->service->ID = 4;
		iss_struct->service->ID_present = false;
	
		iss_struct->service->samp_freq_in_m_sec = 3000;
		iss_struct->service->samp_freq_present = false;
		...
		iss_struct->service->gatt_db_needs_cleaning = false;
		iss_struct->service->p_persist_measurement = persist_measurement;
		
		*err_code = iss_initialize(iss_struct->service);
...
static void services_init(void)
{
    ...
    ble_dis_sys_id_t sys_id;
		...
		if(sensors_size == 1){
			uint16_t sam_freq = sensors[0]->service->samp_freq_in_m_sec / 900; //900 ensures that we time out after the samp freq
...
{
		for(uint8_t i = 0; i < sensors_size; i++){
				sensors[i]->sensor_ble_evt(p_ble_evt);
		}
