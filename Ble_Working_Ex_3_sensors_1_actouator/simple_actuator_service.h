...
typedef struct ias
{
    uint16_t                     service_handle;               /**< Handle of the Service (as provided by the BLE stack). */
		ble_gatts_char_handles_t     actuator_handles;      			 /**< Handles related to the measurement config characteristic. */
    uint16_t                     conn_handle;                  /**< Handle of the current connection (as provided by the BLE stack, is BLE_CONN_HANDLE_INVALID if not in a connection). */
		uint8_t											 status;
		void (*p_update_status)(struct ias *);									 /**< Function to update the sampling frequency if client asks for it*/
} ias_t;
